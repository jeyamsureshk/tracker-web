...
import Chart from 'chart.js/auto'; // Required for chart generation
...
// Initialize Gemini
const genAI = new GoogleGenerativeAI("AIzaSyCLGtQ_WiVyRBHz49AXP1MLpMtg-Eiltn4");
...
// --- Excel Export Function (Matching Utilization Report 2026 Format - ALL DATA) ---
const handleExportToExcel = async () => {
  ...
  const uniqueTeams = [...new Set(productions.map(p => p.team))].filter(Boolean);

  // 1. Generate Individual Team Sheets
  uniqueTeams.forEach((teamName, teamIndex) => {
    const safeSheetName = teamName.substring(0, 31);
    const teamSheet = workbook.addWorksheet(safeSheetName);
    
    // Sort all records for this team by Date, then by Hour
    const teamRecords = productions
    ...
    teamSheet.addRow([teamName]);
    teamSheet.getCell('A1').font = { bold: true, size: 14, color: { argb: blueTheme } };

    // Row 2: Headers (With Date replacing SL.No)
    const headers = [
      'Date', 'Time Duration', 'Model Name', 'Planned Manpower', 'Actual Manpower', 
      'Total Available Time (min)', 'Planned Downtime (min) (Break + Offline Work + Change Over)', 
      'Actual Available Time (min)', 'Un-Planned Downtime (min)', 
      'Actual Run Time (min) = Actual Available Time -  Unplanned Downtime', 
      'Line Utilization (%) = (Actual Run Time / Actual Available Time) * 100', 
      'Line Utilization (%) = (Actual Run Time / Total Available Time) * 100', 
      'Target Output (Qty)', 'Actual Output (Qty)', 
      'Efficiency(%) = (Actual Output / Target Output) * 100', 
      'Good Output(Qty)', 'Quality (FPY) = (Good Output / Actual Output) * 100', 'Remarks'
    ];
    
    const headerRow = teamSheet.addRow(headers);
    headerRow.height = 40;
    ...
    // Column Widths
    teamSheet.columns = [
      { width: 12 }, { width: 14 }, { width: 25 }, { width: 10 }, { width: 10 },
      { width: 12 }, { width: 15 }, { width: 12 }, { width: 15 }, { width: 15 },
      { width: 15 }, { width: 15 }, { width: 10 }, { width: 10 }, { width: 12 },
      { width: 10 }, { width: 12 }, { width: 30 }
    ];

    let totals = { runTime: 0, actualAvail: 0, totalAvail: 0, target: 0, actual: 0, good: 0 };
    let currentManpower = 0;

    // Loop through EVERY record available for the team (No row limits)
    teamRecords.forEach((record, index) => {
      const rowNum = index + 3; // Offset for title and headers

      const modelName = (record.item || []).map((i: any) => i.model).join(', ');
      const actualMp = record.manpower?.toString() || '';
      currentManpower = record.manpower || currentManpower;
      const target = record.target_units?.toString() || '0';
      const actual = record.units_produced?.toString() || '0';
      const remarks = record.remarks || '';
      
      const dts = parseDowntime(remarks);
      const pdt = dts.planned > 0 ? dts.planned.toString() : '';
      const udt = dts.unplanned > 0 ? dts.unplanned.toString() : '';
      const defectQty = parseDefectsFromRemarks(remarks) + (record.defect_qty || 0);

      const formattedDate = new Date(record.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-');

      // Calculate time slot label dynamically
      const h = Math.floor(record.hour);
      const m = Math.round((record.hour % 1) * 60);
      
      // Determine duration logic (e.g. 08:30 is usually a 30min slot, others are 60min)
      const isHalfHourSlot = (h === 8 && m >= 30);
      const totalAvail = isHalfHourSlot ? 30 : 60;
      ...
      if (isHalfHourSlot) {
        ...
        const nextH = h + 1;
        timeLabel = `${h.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')} - ${nextH.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}`;
      }

      const plannedDtVal = pdt ? parseInt(pdt) : 0;
      const actualAvail = totalAvail - plannedDtVal;
      const unplannedDtVal = udt ? parseInt(udt) : 0;
      const actualRun = actualAvail - unplannedDtVal;
      const actQty = actual ? parseInt(actual) : 0;
      const tgtQty = target ? parseInt(target) : 0;
      const goodQty = actQty - defectQty;

      // Accumulate for summary
      totals.runTime += actualRun;
      totals.actualAvail += actualAvail;
      totals.totalAvail += totalAvail;
      totals.target += tgtQty;
      totals.actual += actQty;
      totals.good += goodQty;
      ...
      const row = teamSheet.addRow([
        ...
        timeLabel,
        modelName,
        actualMp, // Planned MP (Using Actual as fallback)
        actualMp, // Actual MP
        totalAvail,
        pdt,
        { formula: `F${rowNum}-G${rowNum}`, result: actualAvail }, // Actual Avail
        udt,
        { formula: `H${rowNum}-I${rowNum}`, result: actualRun }, // Actual Run
        { formula: `IFERROR(J${rowNum}/H${rowNum}, 0)`, result: actualAvail ? actualRun / actualAvail : 0 }, // Line Util (Actual)
        { formula: `IFERROR(J${rowNum}/F${rowNum}, 0)`, result: actualRun / totalAvail }, // Line Util (Total)
        target,
        actual,
        { formula: `IFERROR(N${rowNum}/M${rowNum}, 0)`, result: tgtQty ? actQty / tgtQty : 0 }, // Efficiency
        goodQty || '',
        { formula: `IFERROR(P${rowNum}/N${rowNum}, 0)`, result: actQty ? goodQty / actQty : 0 }, // Quality
        remarks
      ]);

      // Formatting Row
      row.eachCell((cell, colNumber) => {
        cell.border = { top: {style:'thin', color: {argb: borderColor}}, left: {style:'thin', color: {argb: borderColor}}, bottom: {style:'thin', color: {argb: borderColor}}, right: {style:'thin', color: {argb: borderColor}} };
        cell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
        if (colNumber === 11 || colNumber === 12 || colNumber === 15 || colNumber === 17) {
          cell.numFmt = '0.00%'; // Apply percentage format to formula columns
        }
      });
      if (index % 2 === 0) {
        row.eachCell({ includeEmpty: true }, c => c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: lightBlueBg } });
      }
    });

    // Store data for the master summary sheet
    summaryData.push({
      sno: teamIndex + 1,
      stage: teamName,
      ...
      mp: currentManpower,
      utilActual: totals.actualAvail ? totals.runTime / totals.actualAvail : 0,
      utilTotal: totals.totalAvail ? totals.runTime / totals.totalAvail : 0,
      efficiency: totals.target ? totals.actual / totals.target : 0,
      quality: totals.actual ? totals.good / totals.actual : 0
    });
  });

  // 2. Generate Master Summary Sheet
  const summarySheet = workbook.addWorksheet('Summary', { properties: { tabColor: { argb: 'FFC000' } } });
  
  // Shift the worksheet to be the first tab
  ...
  summarySheet.columns = [
    ...
    { width: 8 }, // B SNO
    { width: 20 },// C Stage
    { width: 18 },// D Util Actual
    { width: 18 },// E Util Total
    { width: 14 },// F Efficiency
    { width: 14 },// G Quality Rate
    { width: 3 }, // H (Spacer)
    { width: 8 }, // I SNO
    { width: 20 },// J Stage
    { width: 12 },// K Target
    { width: 12 },// L Actual
    { width: 15 },// M MP Allocation
    { width: 14 },// N Efficiency
    { width: 18 } // O Util Total
  ];

  summarySheet.addRow([]); // Row 1 Empty

  // Row 2: Headers
  const sumHeaders = summarySheet.addRow([
    '', 'SNO', 'Stage', 'Line Utilization %\n Actual Available Time', 'Line Utilization %\nTotal Available Time', 'Efficiency %', 'Quality Rate%',
    '', 'SNO', 'Stage', 'Target', 'Actual', 'Manpower\nAllocation %', 'Efficiency %', 'Line Utilization %\nTotal Available Time'
  ]);
  
  sumHeaders.height = 45;
  sumHeaders.eachCell((cell, colNumber) => {
    if (colNumber !== 1 && colNumber !== 8) { // Skip spacers
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: blueTheme } };
  ...
  // Calculate total MP for allocation percentage
  const totalMp = summaryData.reduce((sum, data) => sum + data.mp, 0);

  // Populate Summary Data
  summaryData.forEach((data, index) => {
    const row = summarySheet.addRow([
      '', 
      data.sno, data.stage, data.utilActual, data.utilTotal, data.efficiency, data.quality, // Left Table
      '', 
      data.sno, data.stage, data.target, data.actual, totalMp ? (data.mp / totalMp) : 0, data.efficiency, data.utilTotal // Right Table
    ]);
    ...
    row.eachCell((cell, colNumber) => {
      if (colNumber !== 1 && colNumber !== 8) {
        ...
        // Apply % formatting
        if ([4, 5, 6, 7, 13, 14, 15].includes(colNumber)) {
          cell.numFmt = '0.00%';
        }
      }
    });
    ...
    if (index % 2 === 0) {
      row.eachCell((cell, colNum) => {
        if (colNum !== 1 && colNum !== 8) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: lightBlueBg } };
      });
    }
  });
  ...
  saveAs(blob, `Utilization_Report_${dateStr}.xlsx`);
};
